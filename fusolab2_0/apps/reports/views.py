...
def excel_writer(request, what, from_day, from_month, from_year, to_day, to_month, to_year):
    ''' generate an excel with the full balance for this reference period '''
    ...
    ### create generic styles for cell content
    ...
    if what == 'bar':
        ...
        for rowx, bb in enumerate(BarBalance.objects.filter(qf).order_by('date'), start=1):
            ...
            sheet.write(rowx, 11, str(data['cashier']))
        ...
        for rowx, bb in  enumerate(BarBalance.objects.filter(qf).order_by('date'), start=1):
            ...
            list_sheet.write(rowx,4,str(bb.cashier))
            list_sheet.write(rowx,5,bb.note,note_format)        
